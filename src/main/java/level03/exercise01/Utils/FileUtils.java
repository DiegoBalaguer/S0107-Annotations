...
public class FileUtils {
    ...
    public static void isCorrectPath(Path directoryPath) throws InvalidPathException {
        if (!Files.exists(directoryPath) || !Files.isDirectory(directoryPath)) {
            throw new InvalidPathException(directoryPath.toString(), "❌ Directory does not exist");
        }
    }

    public static void exist(Path pathWithFilename) {
        if (!pathWithFilename.toFile().exists()) {
                throw new InvalidPathException(pathWithFilename.toString(), "❌ File does not exist");
            }
        }
